      ...
      <Card variant="bordered" padding="md">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {result.missedConcepts.length > 0 && (
                <Badge variant="warning">
                  {result.missedConcepts.length} missed
                </Badge>
              )}
            </div>
          </div>
        ...
        {expanded && (
          <motion.div
            ...
            className="mt-4 space-y-4 border-t border-surface-700 pt-4"
          >
            {/* Your answer */}
            <div>
              <p className="text-xs text-slate-500 mb-2">Your Answer</p>
              <p className="text-slate-400 text-sm font-mono bg-surface-900/50 rounded-xl p-3 leading-relaxed">
                {result.transcript}
              </p>
            </div>
            ...
            {result.missedConcepts.length > 0 && (
              <div>
                ...
                <ul className="space-y-1">
                  {result.missedConcepts.map((m, i) => (
                    <li
                      key={i}
                      className="text-amber-300 text-sm flex items-start gap-2"
                    >
            ...
            <div>
              <p className="text-xs text-slate-500 mb-2 flex items-center gap-1">
                ...
                Refined Perfect Answer
              </p>
              <div className="bg-brand-500/5 border border-brand-500/20 rounded-xl p-4 text-sm text-slate-300 leading-relaxed">
...
function ResultsContent() {
  ...
  const evaluations: EvaluationResult[] = useMemo(() => {
    try {
      return evalsRaw ? JSON.parse(decodeURIComponent(evalsRaw)) : [];
    } catch {
      return [];
    }
  }, [evalsRaw]);
            ...
            <div>
              ...
              <p className="text-slate-400 text-sm mt-1">
                {avgScore >= 8
                  ...
                  : avgScore >= 6
                    ? "Good effort — review the missed concepts."
                    : "Keep practicing to improve your depth."}
              </p>
            </div>
            <div className="flex gap-4 text-center">
              ...
              <div>
                <p className="text-2xl font-bold text-emerald-400">
                  {evaluations.reduce((s, e) => s + e.strengths.length, 0)}
                </p>
              ...
              <div>
                <p className="text-2xl font-bold text-amber-400">
                  {evaluations.reduce((s, e) => s + e.missedConcepts.length, 0)}
                </p>
      ...
      <div>
        ...
        <div className="space-y-3">
          {evaluations.length > 0 ? (
            evaluations.map((e, i) => (
              <QuestionCard key={i} result={e} index={i} />
            ))
          ) : (
...
export default function ResultsPage() {
  return (
    <Suspense
      fallback={<div className="text-slate-400 text-sm">Loading results…</div>}
    >
      <ResultsContent />
    </Suspense>
  );
